...
@RestController("objetoControllerV1")
...
public class ObjetoController {
    ...
    public ResponseEntity<?> buscarPorTipoObjeto(@PathVariable String tipoObjeto) {
        try {
            List<ObjetoDTO> objetos = objetoService.buscarPorTipo(tipoObjeto);
            return ResponseEntity.ok(objetos);
        } catch (IllegalArgumentException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
        } catch (RuntimeException e) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> actualizarObjeto(@PathVariable Long id, @Valid @RequestBody ObjetoDTO datosNuevos) {
        try {
