...
public class EventoService {
    ...
    public Evento guardarEvento(Evento evento) {
        if (eventoValidaciones.validarNullSinNada(evento) == false) {
            throw new IllegalArgumentException("Los datos del evento no pueden ser nulos o están incompletos.");
        }
        String DonLimpio = evento.getDescripcionEspecifica().trim().replaceAll("\\s+", " "); /* esto sirve para limpiar los espacios en blanco */
        evento.setDescripcionEspecifica(DonLimpio);
        return eventoRepository.save(evento);
    }

    public Evento actualizarEvento(Long id, Evento eventoDetalles) {
        if (eventoValidaciones.existeEnBaseDatos(id) == false) {
            throw new RuntimeException("No se puede actualizar. El evento físico no existe con el ID: " + id);
        }
        ...
        if (eventoDetalles.getTipoEvento() != null) { /* aqui empezamos a actualizar wachin */
        ...
        if (eventoDetalles.getDescripcionEspecifica() != null && eventoDetalles.getDescripcionEspecifica().trim().isEmpty() == false) {
    ...
    public Evento editarEvento(Long id, Evento eventito) {
        ...
        if (eventito.getDescripcionEspecifica() != null && eventito.getDescripcionEspecifica().trim().isEmpty() == false) {
