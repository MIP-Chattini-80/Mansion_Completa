...
public class ObjetoService {
    ...
    public List<ObjetoDTO> buscarPorTipo(String tipo) {
        if (tipo == null || tipo.trim().isEmpty()) {
            throw new IllegalArgumentException("El tipo de objeto no puede estar vacío.");
        }
        String tipoSanitizado = tipo.trim();
        return objetoRepository.findByTipoObjeto(tipoSanitizado).stream()
                .map(this::mapToDTO)
                .collect(Collectors.toList());
    }
    ...
    public ObjetoDTO actualizarObjeto(Long id, ObjetoDTO datosNuevos) {
        if (id == null || datosNuevos == null) {
            throw new IllegalArgumentException("El ID o los datos no pueden ser nulos.");
        }
        if (!validaciones.existeEnBaseDatos(id)) {
            throw new RuntimeException("No existe el objeto con el ID: " + id);
        }
