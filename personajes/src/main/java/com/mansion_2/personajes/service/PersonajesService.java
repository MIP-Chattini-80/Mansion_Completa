...
public class PersonajesService {
    ...
    public PersonajesDTO actualizarPersonajes(Long id, PersonajesDTO datosNuevos) { // CAMBIADO A PersonajesDTO
        if (validaciones.existeEnBaseDatos(id) == false) {
            throw new RuntimeException("No se puede actualizar. La categoría no existe con el ID: " + id);
        }
        Personajes objetoParaValidar = new Personajes();
        objetoParaValidar.setCategoria(datosNuevos.getCategoria());

        if (validaciones.validarNullSinNada(objetoParaValidar) == false) {
